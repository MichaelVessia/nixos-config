...
2. Update `sonarr_api_key` in `secrets/framework13.yaml` via:
   `SOPS_AGE_KEY_FILE=~/.config/sops/age/keys.txt sops set secrets/framework13.yaml '["sonarr_api_key"]' '"<new key>"'`
3. Rebuild home-manager and open a fresh shell so the new value is exported.
4. Verify: `curl -v "$SONARR_URL/api/v3/system/status" -H "X-Api-Key: $SONARR_API_KEY"`
