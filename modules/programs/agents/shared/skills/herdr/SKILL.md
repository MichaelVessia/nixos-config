---
name: herdr
description: Inspect or control Herdr panes, workspaces, and agents when explicitly requested from a session with HERDR_ENV=1.
---

# Herdr

Local copy of [Herdr v0.9.0](https://github.com/herdrdev/herdr/blob/v0.9.0/skills/herdr/SKILL.md). After satisfying the environment gate below, read [local placement policy](./placement.md) before creating splits, tabs, or worktrees. It refines the upstream layout guidance with grid-aware splitting, task-based tab names, and nested worktrees. The rest of this file is the upstream CLI reference.
