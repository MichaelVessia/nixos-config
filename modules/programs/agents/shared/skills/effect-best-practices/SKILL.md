...
| **Platform I/O & Config** | [references/platform-io.md](references/platform-io.md) | **No sync I/O, FileSystem service, Config for tunables, selective error catching** |
...
| Sync Node.js I/O (`readFileSync`, `existsSync`, etc.) | Use `FileSystem.FileSystem` from Effect platform (see [references/platform-io.md](references/platform-io.md)) |
| `execSync` / `child_process` sync calls | Use an Effect-tracked shell service or platform `Command` |
| Blanket `Effect.catchAll(() => Effect.succeed(fallback))` | Distinguish expected vs unexpected errors (see [references/platform-io.md](references/platform-io.md)) |
| Hardcoded module-level constants for tunables | Use `Config.string`/`Config.number` with `Config.withDefault` |
